...
class Draw_window(object):
    ...
    # カラム設定
        ...
        col3 =  [   [
                    ...
                    sg.Multiline(size=(20,17), key='-M_BOX_3-',autoscroll=True),                   
                ]   ]
        layout = [            
                    # 丈夫カラム
    ...
    def _window(self):
        ...
        # 折れ線グラフ
            ...
            self._ax5.set_ylim(-5, 500)
            self._ax5.set_xlim(self.xlim, self.x2lim)
        ...
        # 折れ線グラフに発話数のデータを追加する関数
            def append_graph_data(mic = 0, MIXER = 0):
                ...
                self.xlim+=0.5
                self.x2lim+=0.5
                # self._ax5.set_xlim(100, 200)
                self._ax5.set_xlim(self.xlim, self.x2lim)
                # self._fig_agg_5.draw()
        ...
        #イベント設定
            ...
            if event in ('Exit', None):
                ...
                f.write(amount_of_talk_data)
                f.close()
                print("aaaaa")
                # t1.sleeo()
                # t2.sleep()
                exit(69)
        amount_of_talk_data1 = self._ys
        amount_of_talk_data2 = self._y2s
        data = np.c_[self._ys,self._y2s]
        now = datetime.datetime.now().strftime('%Y-%m-%d')
        np.savetxt(LOG_DIRECTORY+now+'amount_of_talk_data.csv',data,delimiter=',')
        # amount_of_talk_data = self._ys
        # f = open(LOG_DIRECTORY+'amount_of_talk_data1.txt', 'w')
        # f.write(amount_of_talk_data1)
        # f.close()
        # print("aaaaa")        
        # f = open(LOG_DIRECTORY+'amount_of_talk_data2.txt', 'w')
        # f.write(amount_of_talk_data2)
        # f.close()
        # print("aaaaa")        
